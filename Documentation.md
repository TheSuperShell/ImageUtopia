...
![Screenshot 2024-01-14 234138.png](Screenshot%202024-01-14%20234138.png)

(#TODO: add prototypes of all flyout menus)
...
Collection - is a folder (created by the app) that contains all the images and metadata.

To create collection User can press a button on the top left of the app and in the flyout
menu choose `create new collection`. (#TODO: add description of collection creation menu).
If the User opens the app for the first time, or previously created and opened collection was
not found, instead of the main window User will see the collection creation window.
...
The menu button is located on the top left of the screen. This is a standard flyout menu with options such as:
...
This is one of the most important feature in this app. 
To filter images by folders or tags User can press the corresponding button in the upper row.
A flyout menu will open where User can choose folders/tags to filter by.
User can choose multiple folders/tags. It is done by clicking a checkbox near the names of the folders/tags.
In the bottom left corner of the flyout menu there is a filter mode switcher.
There are 3 modes: `Any`, `All`, `Exclude`.
- `Any` mode: images that have at least one of the chosen folders/tags will be shown (logical `OR`)
- `All` mode: images that have all of the chosen folders/tags will be shown (logical `AND`)
- `Exclude` mode: images that do not have any of the chosen folders/tags will be shown (logical `NOT`)

(Might be changed in the future: User will have to write a prompt in the flyout menu
where they can specify the folders/tags they want to filter by and the filter mode
in a more flexible way. There will be `&`, `|`, `!` 
operators for `All`, `Any` and `Exclude` modes respectively.
For example: `folder1 & folder2 | folder3` means that image must have `folder1` and `folder2` or `folder3`.
Parentheses will be available as well. For example: `folder1 & (folder2 | folder3)`
means that image must have `folder1` and either `folder2` or `folder3`)
...
User can choose multiple images. In this case the inspector will show combined metadata of all chosen images. 
If the metadata of chosen images is different, the corresponding field will be empty. 
If the metadata of chosen images is the same, the corresponding field will show the metadata.
User can change the metadata of all chosen images at once. 
In this case the metadata of all chosen images will be overwritten with the new metadata (except for folders and tags).
As for folders/tags, User can add or remove a tag/folder to all chosen images at once, but it will not overwrite other folders/tags.

There is an export button at the bottom of the inspector when any amount of images is chosen. 
When pressed, a flyout menu will open where User can choose the export format and the export location.
Chosen images will be copied to the chosen location in the chosen format. (might add more export options in the future)
